...
// ========================================
// Single source of truth for all S3 uploads
// Works consistently across all pages (new, edit, etc.)

// Main upload entry point - use this everywhere
...
async function uploadMultipart(file, signData, progressElement, progressTextElement) {
  ...
  if (!completeResponse.ok) {
    ...
    throw new Error(`Failed to complete multipart upload: ${completeResponse.status} - ${s3ErrorCode}`);
  }
}
...
async function uploadPresignedPost(file, signData, progressElement, progressTextElement) {
  ...
  const uploadPromise = new Promise((resolve, reject) => {
    ...
    xhr.onload = () => {
      if (xhr.status >= 200 && xhr.status < 300) {
        ...
        reject(new Error(`POST upload failed: ${xhr.status} ${xhr.statusText}`));
      }
    };

    xhr.onerror = () => {
      reject(new Error('Network error during POST upload'));
    };
...
window.uploadPresignedPost = uploadPresignedPost;