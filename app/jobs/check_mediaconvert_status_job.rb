class CheckMediaconvertStatusJob < ApplicationJob
  queue_as :default
  
  def perform(program_id)
    program = Program.find(program_id)
    return unless program.mediaconvert_job_id.present?
    return if program.status == 'ready' # Already completed
